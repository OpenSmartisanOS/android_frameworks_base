...
open class ModernStatusBarView(context: Context, attrs: AttributeSet?) :
    ...
    private fun initDotView() {
        ...
        val width = getStatusBarDotSize()
        val lp = LayoutParams(width, width)
        lp.gravity = Gravity.CENTER_VERTICAL or Gravity.START
        addView(dotView, lp)
    }

    /** Lets a presentation-specific subclass match its fixed icon geometry. */
    protected open fun getStatusBarDotSize(): Int =
        mContext.resources.getDimensionPixelSize(R.dimen.status_bar_icon_size_sp)
}
